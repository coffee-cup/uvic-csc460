...
Roomba::Roomba(uint16_t serial_connector, uint16_t brc_pin) {
...
bool Roomba::check_power(uint16_t *power) {
    //Serial.printtln("cp");
    uint8_t battery_charge_high = 0;
    uint8_t battery_charge_low = 0;

    while (read_serial(&battery_charge_high))
        ;
    write_serial(COMMAND::SENSORS);
    write_serial(25);
...
bool Roomba::read_serial(uint8_t *val) {
    switch (m_serial_num) {
        case 0:
            if (Serial.available()) {
                *val = Serial.read();
...
void Roomba::write_serial(uint8_t val) {
    switch (m_serial_num) {
    case 0:
        Serial.write(val);
        break;
